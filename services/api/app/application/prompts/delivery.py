...
- 提供的参考信息都已通过 `ref_n` 的方式编号，你需要在正确、合理的位置根据实际使用的参考信息，使用标准 footnotes 语法插入角标，示例：`[^ref_n]`，在文末列出对应的来源，并使用链接语法展示，示例： `[^ref_n]: [title](link)` 。请保证脚注、来源的正确使用和对应关系的准确无误。
- 请根据研究复杂度和大纲，合理规划报告篇幅，避免输出过长。
