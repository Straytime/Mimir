...
2. 流式输出模型响应展示给用户
3. 用户通过输入框提供澄清反馈，提交后，直接基于用户反馈内容原文进入需求分析（func_6）
...
1. 按上述“有序列表为问题、问题后无序列表为选项”的预期格式，捕获对应内容，并通过交互式组件展示给用户，供用户直接点选
    1. 在模型输出选单基础上，固定在每个选项列表末尾增加 `自动` 选项
    2. 所有题目，默认选择 `自动` 选项
    3. 选单澄清过程中，不接受输入框输入
2. 从选单展示开始，倒计时 15s，自动进入需求分析（func_6）
...
2. web_search 工具
    ...
    - 响应样例如下，返回 tool message 时，仅保留 `search_result` 部分，同时去除 `icon` `media`
