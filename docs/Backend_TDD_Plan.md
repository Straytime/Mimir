...
3. `clarification natural` 与 `clarification options` 必须断言 PRD 最新的 system / user split：system prompt 承载稳定角色与“一次 `web_search` 上限”规则，user prompt 只承载用户初始需求与当前时间。
...
2. no-tool 阶段不应意外挂载 tool schema；`clarification natural` / `clarification options` 是例外，必须显式挂载 `web_search`
...
1. `clarification func_4 / func_5` 与 `collector func_8` 必须共用同一 `web_search` tool result envelope：
   - 顶层：`success`、`search_query`、`search_recency_filter`、`results`
   - `results[]`：`title`、`link`、`snippet`、`publish_date`
   - 失败时允许追加可选 `error_code`
2. `web_search` tool result 必须保留 provider `publish_date`；缺失时统一输出 `null`，并继续剔除 `icon`、`media`、`search_intent` 等展示性字段
3. clarification 阶段必须覆盖标准 `web_search` tool-call loop：assistant tool_calls -> 执行一次 `web_search` -> tool message 回灌 -> 下一轮最终澄清输出；后端必须显式限制总调用上限为 `1`
4. `web_fetch` 正文截断到前 `5000` 字符，标题提取规则稳定；adapter 与 collection service 若存在双重截断，必须共用同一个配置源
5. `python_interpreter` 只返回文本摘要与 artifact 元数据，不把二进制内容写入 transcript
6. provider 风控、超时、空内容、4xx、5xx 都会被映射到统一异常或统一 tool result envelope
7. E2B `execute` 成功返回但 `execution.error != null` 时，必须映射成 writer 可消费的结构化失败 tool result，而不是 `RetryableOperationError`
8. 自定义 E2B template 的结构测试必须锁定：模板定义文件存在、字体资产存在、模板 build 步骤会安装 `Noto Sans CJK SC` 并刷新字体缓存
...
- PDF 主路径必须采用标准 `GFM -> HTML -> PDF` 渲染，而不是继续使用手写 HTML 节点翻译器逐项补丁；当前实现口径应收敛为 headless Chromium 打印 HTML
